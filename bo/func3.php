...
{  
   ...
   // echo '<input type="submit" name="submit" value="Submit" />';   
    echo "<script type=\"text/javascript\">
$('#wait_12').hide();
	$('#drop_12').change(function(){
	  $('#wait_12').show();
	  $('#result_12').hide();
      $.get(\"func3.php\", {
		func: \"drop_12\",
		drop_var: $('#drop_12').val()
      }, function(response){
        $('#result_12').fadeOut();
        setTimeout(\"finishAjax_tier_thirteen('result_12', '\"+escape(response)+\"')\", 400);
      });
    	return false;
	});
</script>";  
...
if (isset($_GET['func'])) {
if($_GET['func'] == "drop_12" && isset($_GET['func'])) { 
   drop_12($_GET['drop_var']);
} 
}

function drop_12($drop_var)
{  
    echo '<select name="drop_12_1" id="drop_12_1">
	      <option value=" " disabled="disabled" selected="selected">Velg</option>';
	...
	while ( $drop_13 = $result->fetch_array() ) { 
			  echo '<option value="'.$drop_13[0].'">'.$drop_13[1].'</option>';
			}
