...
{  
   ...
   echo "<script type=\"text/javascript\">
$('#wait_16').hide();
	$('#drop_16').change(function(){
	  $('#wait_16').show();
	  $('#result_16').hide();
      $.get(\"func4.php\", {
		func: \"drop_16\",
		drop_var: $('#drop_16').val()
      }, function(response){
        $('#result_16').fadeOut();
        setTimeout(\"finishAjax_tier_seventeen('result_16', '\"+escape(response)+\"')\", 400);
      });
    	return false;
	});
</script>";   
...
if (isset($_GET['func'])) {
if($_GET['func'] == "drop_16" && isset($_GET['func'])) { 
   drop_16($_GET['drop_var']);
} 
}

function drop_16($drop_var)
{  
    echo '<select name="drop_16_1" id="drop_16_1">
	      <option value=" " disabled="disabled" selected="selected">Velg</option>';
	...
	while ( $drop_17 = $result->fetch_array() ) { 
			  echo '<option value="'.$drop_17[0].'">'.$drop_17[1].'</option>';
			}
