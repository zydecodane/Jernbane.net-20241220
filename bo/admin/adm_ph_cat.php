	...
	<br /><br />
			...
			if ($rs == 1) {$img[6] = 0; $img[7] = 0; $img[8] = 0;}
			  ...
			  { 	
				...
				$skat = $result->fetch_array();
			  	...
			  	$query = "select katname, natnavn, natID from gal_kategori where katid = '$skat[0]'";
				$result = $db->query($query);
			 ...
			 ?>
			  <select name="drop_1" id="drop_1">
			    <option value="" <?php if( empty($val) ) {echo  'selected="selected"';} ?> disabled="disabled">Velg Land</option>
			     <?php
			     getTierOne($val); 
			     ?>
		...
		<!-- ################################ KATEGORI ###########################  -->
			<div class="cat_left">
			 ...
			 <?php
			  if ($img[6] != '0')
			  {
				  if($en == '1' && !empty($stype[2]) ) {	
			 ?>
 			   <select name="drop_2" id="drop_2">
			    <option value="" <?php if( empty($stype[2]) ) {echo  'selected="selected"';} ?> disabled="disabled">Velg kategori</option>
			     <?php
			     getTierTwo($stype[2], $skat[0]); 
			     ?>
			  </select>&nbsp;<span id="wait_2" style="display: none;">&nbsp;
			    <img alt="Please Wait" src="../graphics/load.gif"/></span>
			 <?php
			 } else {
			  	      echo $stype[0];
		...
		<!-- ################################ TYPE / BANE ###########################  -->
			 ...
			 <div class="cat_right">
				 <?php
				  if ($img[6] != '0')
				  {
				    if($en == '1' && !empty($skat[1]) ) {	
			 ?>
 			   <select name="drop_3" id="drop_3">
			    <option value="" <?php if( empty($skat[1]) ) {echo  'selected="selected"';} ?> disabled="disabled">Velg type/bane</option>
			     <?php
			     getTierThree($skat[0], $img[6]); 
			     ?>
			  </select>&nbsp;<span id="wait_3" style="display: none;">&nbsp;
			    <img alt="Please Wait" src="../graphics/load.gif"/></span>
			 <?php
			       } else {
				  	   echo $skat[1];
		...
		<!-- ################################ NUMMER / STED ###########################  -->
			 ...
			 ?>
			    ...
			    <option value="" <?php if( empty($img[7]) ) {echo  'selected="selected"';} ?> disabled="disabled">Velg nummer/sted</option>
			     <?php
			     getTierFour($img[6], $img[7]); 
			     ?>
			  </select>&nbsp;<span id="wait_4" style="display: none;">&nbsp;
			    <img alt="Please Wait" src="../graphics/load.gif"/></span>
			 <?php
			       } else {
						  		echo $sunit[0]; 
		...
		<!-- ################################ DETALJ ###########################  -->
			 ...
			 ?>
			    ...
			    <option value="" <?php if( empty($img[7]) ) {echo  'selected="selected"';} ?> disabled="disabled">Velg detalj</option>
			     <?php
			     getTierFive($img[7], $img[8]); 
			     ?>
		...
		<!-- ##########################################################  -->
				...
				<form name="catendre" style="display: inline-block; vertical-align: top;" method="post" action="index.php?s=6&id=<?php echo $id; ?>&e=1&page=<?php echo $page; ?>&eg=<?php echo $eg.$ae_parstr; ?>&fi=<?php echo $fi; ?>">   
				   <input type="hidden" name="logid" value="<?php echo $logid; ?>" />
				   <input type="submit" value="Endre" style="width: 100px;" />
				</form>   
				<form name="catreset" style="display: inline-block; vertical-align: top;" method="post" action="index.php?s=6&id=<?php echo $id; ?>&r=1&page=<?php echo $page; ?>&eg=<?php echo $eg.$ae_parstr; ?>&fi=<?php echo $fi; ?>">   
				   <input type="hidden" name="logid" value="<?php echo $logid; ?>" />
