...
function getTierOne($sel = '')
{
	...
	while ($tier = $result->fetch_array()) 
		{
			$seltxt = '';
			if(!empty($sel) &&  $tier[2] == $sel) {
				$seltxt = ' selected="selected"';
			}
		   echo '<option value="'.$tier[0].'"'.$seltxt.'>'.$tier[2].'</option>';
		}

}

function getTierTwo($nat, $sel = '')
{
	include ('../configi.php');
	
    $query = "select * from gal_kategori where natID = '$nat' order by 4";
	$result = $db->query($query);
	while ($tier = $result->fetch_array()) 
		{
			$seltxt = '';
			if(!empty($sel) &&  $tier[0] == $sel) {
				$seltxt = ' selected="selected"';
			}
		   echo '<option value="'.$tier[0].'"'.$seltxt.'>'.$tier[1].'</option>';
		}

}

function getTierThree($kat, $sel = '')
{
	include ('../configi.php');
	
    $query = "select * from gal_type where katid = '$kat' order by 5";
	$result = $db->query($query);
	while ($tier = $result->fetch_array()) 
		{
			$seltxt = '';
...
function getTierFour($typeid, $sel = '')
{
	include ('../configi.php');
	
    $query = "select * from gal_unit where typeid = '$typeid' order by 3";
	$result = $db->query($query);
	while ($tier = $result->fetch_array()) 
		{
			$seltxt = '';
...
function getTierFive($unitid, $sel = '')
{
	include ('../configi.php');
	
    $query = "select * from gal_unitdetail where numid = '$unitid' order by 4";
	$result = $db->query($query);
	while ($tier = $result->fetch_array()) 
		{
			$seltxt = '';
